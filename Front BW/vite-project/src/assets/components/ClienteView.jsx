...
const misLibros = [
  {
    ...
    stock: 18,
    cover:
      "https://i.pinimg.com/736x/e9/fd/20/e9fd20aa26f6c54361b6692d85e2c795.jpg",
  },
  {
    id: 101,
    ...
    stock: 27,
    cover:
      "https://i.pinimg.com/736x/a6/39/13/a639138484094c40856b464389acf3b3.jpg",
  },
  {
    id: 102,
    ...
    stock: 14,
    cover:
      "https://i.pinimg.com/736x/7b/65/2c/7b652ccb6c0b4c59b63191fb7b7d4c88.jpg",
  },
];
...
export default function ClienteView({ onLogout }) {
  ...
  const [datosUsuario, setDatosUsuario] = useState({
    ...
    ultimoPedido: "Ninguno",
    estadoPedido: "N/A",
    generosFavoritos: [],
  });

  const [datosTemp, setDatosTemp] = useState({ ...datosUsuario });

  const abrirEdicion = () => {
    setDatosTemp({ ...datosUsuario });
    setEditando(true);
  };
  ...
  const handlePerfilChange = (e) => {
    ...
    setDatosTemp((prev) => ({ ...prev, [name]: value }));
  };
  // -----------------------------------------
  ...
  const authors = [...new Set(books.map((book) => book.autor).filter(Boolean))];

  const filteredBooks = books.filter((book) => {
    const matchesText = `${book.nombre} ${book.autor} ${book.genero}`
      .toLowerCase()
      .includes(filter.toLowerCase());
    const matchesGenre = !genreFilter || book.genero === genreFilter;
    const matchesAuthor = !authorFilter || book.autor === authorFilter;
    const matchesPrice = !maxPrice || Number(book.precio) <= Number(maxPrice);
    return matchesText && matchesGenre && matchesAuthor && matchesPrice;
  });
  ...
  return (
    <>
      ...
      <div
        className={`perfil-overlay ${perfilAbierto ? "activo" : ""}`}
        onClick={() => {
      ...
      <aside className={`perfil-panel ${perfilAbierto ? "abierto" : ""}`}>
        ...
        <div className="perfil-contenido">
          {!editando && (
            <>
              ...
              <div className="perfil-seccion">
                <h3>Datos de Envío</h3>
                <p>
                  <strong>Ubicación:</strong> {datosUsuario.ubicacion}
                </p>
              </div>
              ...
              <div className="perfil-seccion">
                <h3>Mi Actividad</h3>
                <p>
                  <strong>Último pedido:</strong> {datosUsuario.ultimoPedido}
                </p>
                <p>
                  <strong>Estado:</strong>{" "}
                  <span className="badge-estado">
                    {datosUsuario.estadoPedido}
                  </span>
                </p>
                <br />
                <p>
                  <strong>Mis géneros:</strong>{" "}
                  {datosUsuario.generosFavoritos.length > 0
                    ? datosUsuario.generosFavoritos.join(" • ")
                    : "—"}
                </p>
              </div>
          ...
          {editando && (
            <>
              ...
              <div className="perfil-seccion">
                <h3>Datos de Envío</h3>
                <label className="perfil-label">Ubicación</label>
                <input
                  className="perfil-input"
                  name="ubicacion"
                  value={datosTemp.ubicacion}
                  onChange={handlePerfilChange}
              ...
              <div className="perfil-seccion">
                ...
                <label className="perfil-label">
                  Separados por coma (ej: Ficción, Poesía)
                </label>
                <input
                  className="perfil-input"
                  name="generosFavoritos"
                  value={datosTemp.generosFavoritos.join(", ")}
                  onChange={(e) =>
                    setDatosTemp((prev) => ({
                      ...prev,
                      generosFavoritos: e.target.value
                        .split(",")
                        .map((g) => g.trim())
                        .filter(Boolean),
                    }))
                  }
                  placeholder="Ficción, Poesía, Aventuras..."
                />
              </div>
      ...
      <footer className="app-footer">
        <div className="footer-container">
          <div className="footer-info">
            ...
            <p>
              Somos 4 estudiantes de Anima Finest apasionados por la lectura.
              Creamos este proyecto para conectar historias con lectores.
            </p>
            <button
          ...
          <div className="footer-info">
            ...
            <p>
              Fomentar el hábito de la lectura mediante un espacio digital
              accesible, cercano y diseñado con el corazón.
            </p>
            <button
              className="footer-btn"
              onClick={() =>
            ...
            <button
              ...
              onClick={() =>
                alert("Puedes contactarnos en: contacto@libreriaindice.com")
              }
            >
              Contacto
            </button>
